...
burden_levels_df <- tibble::tibble(
  ...
  dplyr::mutate(
    Level = factor(Level, levels = c("high", "medium", "low", "very low")),
    Method = factor(Method, levels = c("Intensity levels", "Peak levels")),
...
y_tics <- c(y_tics[-5], rounded_max_threshold, rounded_max_threshold + 3000)
...
all_observations <- dplyr::bind_rows(data_list, .id = "source") |>
  ...
  dplyr::rename(Data = source) |>
  dplyr::mutate(
    Data = factor(
      dplyr::case_when(
        Data == "tsd_data_noise" ~ "Noise",
        Data == "tsd_data_noise_and_pos_trend" ~ "Noise and positive trend",
        Data == "tsd_data_noise_and_neg_trend" ~ "Noise and negative trend",
        TRUE ~ Data
      ),
      levels = c("Noise", "Noise and positive trend", "Noise and negative trend")
    )
  )
...
all_observations |>
  ...
  ggplot2::facet_grid(Method ~ Data) +
  ggplot2::theme_minimal() +
  ggplot2::theme(
    legend.position = "right",
    legend.key.width = grid::unit(2, "cm"),
...
mem_levels_df <- tibble::tibble(
  ...
  Data = c(
    rep("Noise", 5),
    rep("Noise and positive trend", 5),
    rep("Noise and negative trend", 5)
  )
) |>
  dplyr::mutate(
    Level = factor(Level, levels = c("very high", "high", "medium",
                                     "epidemic threshold \n (start)", "epidemic threshold \n (end)")),
    Method = factor(Method, levels = c("mem levels")),
    Data = factor(Data, levels = c("Noise", "Noise and positive trend", "Noise and negative trend"))
  )
...
all_observations |>
  ...
  ggplot2::guides(linetype = "none")
```
In the plots, observations below 100 are filtered out to improve visualization.
...
|---------------------------|-------------|
| Noise                     |    4129     |
| Noise and positive trend  |    5590     |
| Noise and negative trend  |     781     |
