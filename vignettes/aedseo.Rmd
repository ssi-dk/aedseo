...
significant_vs_obs |>
  ...
  ggplot2::labs(
    ...
    linetype = "",
    x = "Rolling sum of positive cases (one time-step)"
  ) +
...
```
The `sum_of_cases` variable is divided with five (as we are using a five-week window) to get the disease-specific
threshold for one time-step.

From the analysis plot above, it is observed that the sequences of observations that have the longest subsequent significant 
growth rates begins at approximately 70 (season *2022/2023*) and 50 (season *2023/2024*) observations.
The lowest number is selected to capture the season onset as early as possible.

In other words, a five-week average observation count surpassing 50 alongside with a significant positive growth rate 
...
By inspecting the output from the above code, the disease-specific threshold is established at `50` observations.
