...
The `time` argument is always required, regardless of whether the observations are cases, incidences, or proportions.
...
```
The seasonal onset alarm for the quasi-Poisson model is in the week between the two binomial models and the disease 
threshold should be divided by the number of samples (250) to compare which also places this in between. 
So in this case where the number of samples is constant over time the different models yield comparable results.
In a more real life setting where the number of samples changes over time it is important to know the reason for the
changes. If it reflects changes in surveillance effort, e.g. variations in number of contribution doctors in a 
sentinel system,  then the (quasi-)binomial family should be preferred. If the reason is that more people are sick 
and a constant fraction of all individuals with symptoms are tested then the (quasi-)Poisson family is 
