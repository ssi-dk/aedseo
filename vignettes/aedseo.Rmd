...
overdispersed binomial data. Supplying `samples` makes `generate_seasonal_data()`
return the observed proportion in addition to the numerator and denominator;
`noise_overdispersion = 5` generates beta-binomial variation and therefore makes
the data suitable for demonstrating a quasi-binomial analysis.
...
Seasonal onset output from the quasi-binomial analysis can be extracted by:

```{r}
summary(seasonal_output_quasibinomial$onset_output)
```

Seasonal burden output from the quasi-binomial analysis can be extracted by:

```{r}
summary(seasonal_output_quasibinomial$burden_output)
```

The two analyses can be compared to see how accounting for overdispersion affects the
estimates.
...
```{r, dpi=300}
plot(
  x = seasonal_output_quasibinomial,
  y_lower_bound = 0.01,
  time_interval = "3 weeks"
)
```
