...
2. The average observation count over the past $k$ units of time exceeds a disease-specific threshold.
...
Criteria two is fulfilled if the `average_observation_warning` in the output is `TRUE`.
...
Given that number of cases are integer-valued, the proposed method relies on generalized linear models (GLM). For count data,
the Poisson distribution is a suitable choice as a model. Hence, the cases denoted as $Y$ are assumed to follow a Poisson distribution
...
For binomial/proportional observations, create the `tsd` with `cases`/`samples` or `proportion`/`samples` and use `family = "binomial"` or `family = "quasibinomial"`. The model then uses `samples` as the denominator (cases out of samples), and the rolling average is reported as a pooled proportion.
...
First we generate cases in a `tsd` object, with the `generate_seasonal_data()` function.
```{r}
...
`na_fraction_allowed = 0.4` defines how large a fraction of cases in the `k` window that are allowed to be `NA`, here `0.4*5 = 2` cases.
...
`season_start` and `season_end` can be used to specify the season to stratify the cases by.
This algorithm runs across seasons, such that the first cases in a new season will use the last `k-1` cases from the previous season.
The `seasonal_onset()` function provides a `tsd_onset` object with a comprehensive seasonal onset analysis.

*NOTE: *As default the output is in cases, but if `incidence` and `population` is in the `tsd` object the average observations will be in incidence and the model will use `population` as offset in each time point.
...
In the first figure, cases over time are shown with a legend for the seasonal onset alarm.
...
Following is an example of predicted cases for the next 5 weekly time steps.
...
The resulting `tsd_predict` object contains case estimates, lower bounds, and upper bounds for each time step.
