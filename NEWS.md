...
Attributes; `outcome_type`, `model_outcome` and `burden_outcome` have been added to object classes of `to_time_series()`, `seasonal_onset()` and `seasonal_burden_levels()` to always be able to track if the results are based on count/incidence or binomial data (#100, #101).
* `summary.tsd_onset()` now reports proportional observations on the proportion scale and prints an explicit `NA` reference when no seasonal onset was detected, instead of producing no output.
